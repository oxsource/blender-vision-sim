...
def export_materials(context, settings, directory: str, samples: int = 64,
    ...
    """Render the four cameras, detect their corners and write the parameters."""
    os.makedirs(directory, exist_ok=True)
    written: List[str] = []

    written += io_mod.render_cameras(context, settings, directory, samples=samples)

    # fill points_2d from the rendered images before writing the skeleton
    from . import corners
    _, corners_status = corners.detect_directory(settings, directory)
    settings.corners_status = corners_status
...
def _filament_skeleton(settings, name: str) -> Dict:
    """A ``vehicle_avm_*.json`` config: ``points_3d`` from the field equation and
    ``points_2d`` from the in-scene corner detector (empty when detection has not
    run or failed, so the external detector can still fill them).

    ``K`` is the effective intrinsics at the camera's output resolution, so it
    matches the rendered image the points were detected in."""
    field = settings.field_spec()
    geo = avm_layout.geometry(field)
    cameras = []
    for record in io_mod.camera_records(settings):
        width, height = _output_size(settings, record["name"])
        fx, fy, cx, cy = _effective_k(settings, record, width, height)
        cameras.append({
            "name": record["name"],
            "points_2d": _points_2d(settings, record["name"]),
    ...
    return {
        "comment": "# AUTO GENERATED BY VISION SIM AVM SCENE "
                   "(points_2d detected from the rendered camera images)",
        "name": name,
        "cameras": cameras,
...
def _effective_k(settings, record: Dict, width: int, height: int):
    """The camera's ``K`` at ``width x height`` (the rendered image size)."""
    ...
    intrinsics = apply_mod.effective_intrinsics(
        camera.data.opencv_cam, int(width), int(height))
    return intrinsics.fx, intrinsics.fy, intrinsics.cx, intrinsics.cy


def _points_2d(settings, camera_name: str) -> List[List[float]]:
    """The detected corners of one camera as ``[[u, v], ...]``, or ``[]``."""
    entry = settings.camera(camera_name)
    if entry is None or not entry.points_2d_ok:
        return []
    values = list(entry.points_2d)
    return [[round(float(values[2 * index]), 3), round(float(values[2 * index + 1]), 3)]
