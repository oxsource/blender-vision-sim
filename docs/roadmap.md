...
| M5d ✅ | **Drive Scene**：室内停车场行驶素材（前摄逐帧渲染 + 每帧车速/位姿真值 + `clip.json`）| 见 [`drive-scene.md`](drive-scene.md)：`[Export Clip]` 出 PNG 序列 + mp4 + 逐帧真值（车体与相机位姿）打包成 zip，关键帧与纯 Python 运动模型逐帧一致 |
